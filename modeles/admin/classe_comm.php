<?php 
	class commande
	{
		...
		public function __construct($id_commande,$nom_commande,$mois_commande,$annee_commande,$date_commande,$photo,$paye,$id_client)
					{
		...
		public function ModifierCommande(){
			if($this->photo==""){
					$bdd = connexionBdd();
					$request=$bdd->prepare("UPDATE `commande` SET `nom_commande`=?,`mois_commande`=?,`annee_commande`=?,`date_commande`=?,`paye`=?,`id_client`=? WHERE id_commande =?");
					...
					$params=array($nom_commande,$mois_commande,$annee_commande,$date_commande,$paye,$id_client,$id_commande);
				...
				$request=$bdd->prepare("UPDATE `commande` SET `nom_commande`=?,`mois_commande`=?,`annee_commande`=?,`date_commande`=?,`photo`=?,`paye`=?,`id_client`=? WHERE id_commande =?");
							$nom_commande		= $this->nom_commande;
							$mois_commande		= $this->mois_commande;
							$annee_commande		= $this->annee_commande;
							$date_commande	= $this->date_commande;
				...
				$params=array($nom_commande,$mois_commande,$annee_commande,$date_commande,$photo,$paye,$id_client,$id_commande);
				$request->execute($params);
		...
		public function affichageCommande(){
			$bdd = connexionBdd();
			$requete = $bdd->prepare("SELECT * FROM `commande` INNER JOIN client ON commande.`id_client` = client.`id_client` ORDER BY id_commande DESC LIMIT 20");
			$requete->execute();
		...
		public function affichageCommandeTri(){
			...
			$requete = $bdd->prepare("SELECT * FROM `commande` INNER JOIN client ON commande.`id_client` = client.`id_client` WHERE mois_commande='$mois' AND annee_commande='$annee' AND paye='Non'");
			$requete->execute();
